  ...
  if (isPdfFile(src)) {
    return (
      ...
      >
        ...
        <a href={src} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-red-700 hover:underline text-center">
          PDF Görüntüle
        </a>
        <span className="text-xs text-gray-500 truncate max-w-full">{getFileName(src)}</span>
      </div>
    );
  }

  if (fill) {
    return <Image src={src} alt={alt} fill className={className} unoptimized sizes="(max-width: 768px) 100vw, 33vw" />;
  }

  return (
    <div className="relative w-full" style={{ height }}>
      <Image src={src} alt={alt} fill className={className} unoptimized sizes="(max-width: 768px) 100vw, 33vw" />
    </div>
  );
}
